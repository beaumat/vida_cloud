              ...
              <table class="table table-sm table-bordered table-hover">
                  <thead class="bg-sky text-xs">
                      <tr>
                          ...
                          <th class="col-2 ">AR/OR Number</th>
                          <th class="col-1 "> Code</th>
                          <th class="col-1 "> Date</th>
                          <th class="">Item Name</th>
                          <th class="text-right col-1">Usage</th>
                          <th class="text-right col-1">Cash</th>
                      ...
                      @foreach ($dataList as $list)
                          @php
                              if ($list->DEPOSIT_AMOUNT > 0) {
                                  ...
                                  $BALANCE = $BALANCE - (float) $list->CREDIT_AMOUNT;
                              }
                          @endphp
                          <tr>
                              <td>
                              ...
                              <td>{{ $list->CREDIT_AMOUNT > 0 ? $list->TRANS_CODE : '' }}</td>
                              <td>
                                  @if ($list->DEPOSIT_AMOUNT == 0)
                                      <a target="_BLANK"
                              ...
                              <td>{{ $list->DEPOSIT_AMOUNT > 0 ? date('m/d/Y', strtotime($list->TRANS_DATE)) : date('m/d/Y', strtotime($list->TRANS_DATE)) }}
                              </td>
                              <td>{{ $list->ITEM_NAME }}</td>
                              <td class="text-right">
                                  {{ $list->DEPOSIT_AMOUNT > 0 ? number_format($list->DEPOSIT_AMOUNT, 2) : '' }}</td>
                              <td class="text-right">
                                  {{ $list->DEPOSIT_AMOUNT > 0 ? '' : number_format($list->CREDIT_AMOUNT, 2) }}</td>
